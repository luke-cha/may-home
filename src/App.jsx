...
function Shop({ lang, detail }) {
  ...
  return <div className="page fade-in shop-page">
    ...
    <section className="shop-gallery-bridge container"><div className="shop-gallery-bridge-copy"><span className="eyebrow">Artificial Flower Gallery</span><h2>{ko ? <>메이플레르만의 다양한 프리미엄 조화 컬렉션을 만나보세요.</> : <>Discover Mayfleur’s distinctive range of premium artificial flower collections.</>}</h2><p>{ko ? '더 많은 디자인 작품을 갤러리에서 확인하실 수 있습니다.' : 'Explore more design works in the gallery.'}</p><a className="text-link" href="#gallery/artificial">{ko ? '조화 갤러리 보기' : 'View Artificial Flower Gallery'} →</a></div><div className="shop-gallery-bridge-images">{imagesOnly(galleryGroups.artificial).slice(0, 4).map((file, i) => <a href="#gallery/artificial" key={file}><Media file={file} alt={`${ko ? '조화 갤러리' : 'Artificial flower gallery'} ${i + 1}`} /></a>)}</div></section>
