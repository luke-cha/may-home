...
const projectCopy = {
  'Gimpo Airport': { ko: '김포공항 국제선 터미널에서 한국의 정서와 자연의 색을 담아낸 3주간의 플로럴 설치 프로젝트입니다.', en: 'A three-week floral installation for Gimpo Airport International Terminal, inspired by Korean heritage and natural colour.' },
  ...
  'Taiwan Workshop': { ko: '메이플레르의 색과 계절에 대한 접근을 나눈 대만 플라워 워크숍입니다.', en: 'A Taiwan workshop sharing Mayfleur’s approach to colour and the seasons.' },
...
function Footer({ lang }) {
  return <footer>
    <div className="footer-main">
      ...
      <div><span className="eyebrow">{lang === 'ko' ? '연결' : 'Connect'}</span><nav><a href="https://instagram.com/mayfleur" target="_blank" rel="noreferrer">{lang === 'ko' ? '인스타그램' : 'Instagram'}</a><a href="mailto:hello@mayfleur.com">{lang === 'ko' ? '이메일' : 'Email'}</a></nav></div>
    </div>
...
function ProductGuide({ lang }) {
  ...
  const items = ko ? [
    ...
    ['배송', '부케·화병·리스는 전국 택배가 가능하며 센터피스·꽃바구니는 서울·경기 차량 배송으로 진행합니다.'],
...
function Gallery({ lang }) {
  ...
  return <div className="page fade-in"><PageHead eyebrow={`— ${ko ? '갤러리' : 'Gallery'}`} title="A Visual Archive of Mayfleur" sub={ko ? '꽃, 오브제, 계절 그리고 순간들' : 'Flowers, objects, seasons and moments'} note={ko ? '생화 · 공간 스타일링 · 워크샵 · 순간 · 작업 과정 모음집' : 'Fresh flowers · spatial styling · workshops · moments · process'} />
    <section className="container"><div className="filter-chips">{[['all', ko ? '전체' : 'All'], ['works', ko ? '플로럴 작품' : 'Floral Works'], ['spaces', ko ? '공간 스타일링' : 'Space Styling']].map(([id, text]) => <button key={id} className={filter === id ? 'active' : ''} onClick={() => setFilter(id)}>{text}</button>)}</div>
      <div className="masonry">{list.slice(0, limit).map((file, i) => <figure key={file}><Media file={file} alt={`Mayfleur gallery ${i + 1}`} /></figure>)}</div>
