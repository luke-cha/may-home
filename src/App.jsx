...
function ShopQuickDeliveryGuide({ lang }) {
  const ko = lang === 'ko'
  return <section className="shop-quick-delivery container"><span className="eyebrow">Delivery</span><h2>{ko ? '카카오 T 퀵 배송비 안내' : 'Kakao T Quick Delivery Fees'}</h2><p>{ko ? '조화와 생화에 동일한 지역별 배송비 및 주문 금액별 배송비 지원 기준이 적용됩니다.' : 'The same regional fees and order-based delivery benefits apply to artificial and fresh flower orders.'}</p><QuickDeliveryFeeGuide ko={ko} /></section>
}
...
function EditorialProductDetail({ product, editorial, lang, guide }) {
  ...
  return <div className="page fade-in product-detail editorial-product-detail container"><a className="back" href="#shop">← {ko ? '샵으로 돌아가기' : 'Back to Shop'}</a>
    ...
    {(editorial.delivery || editorial.custom) && <section className="editorial-delivery-custom">{editorial.delivery && <article><span className="eyebrow">Delivery</span>{paragraphs(editorial.delivery)}<details className="contact-delivery-fees product-delivery-fees"><summary>{ko ? '카카오 T 퀵 배송비 보기' : 'View Kakao T Quick Delivery Fees'} <b aria-hidden="true" /></summary><QuickDeliveryFeeGuide ko={ko} /></details></article>}{editorial.custom && <article><span className="eyebrow">Custom Option</span>{paragraphs(editorial.custom)}</article>}</section>} 
    <OrderGuide guide={guide} />
  </div>
}
...
function QuickDeliveryFeeGuide({ ko }) {
  return <div className="contact-delivery-fee-panel"><div className="fresh-delivery-fees"><h3>Delivery Fee</h3><dl>{quickDeliveryFees.map(([price, areasKo, areasEn]) => <div key={price}><dt>{ko ? price : `KRW ${price.replace('원', '')}`}</dt><dd>{ko ? areasKo : areasEn}</dd></div>)}</dl><p>{ko ? '표기되지 않은 지역은 배송지 확인 후 안내드립니다.' : 'For areas not listed, the fee will be confirmed after checking the delivery address.'}</p></div><div className="fresh-delivery-benefit"><h3>Delivery Benefit</h3><p><strong>{ko ? '20만원 이상 주문 시 배송비 최대 15,000원 지원' : 'Orders over KRW 200,000 receive up to KRW 15,000 in delivery support.'}</strong></p><p><strong>{ko ? '30만원 이상 주문 시 배송비 최대 20,000원 지원' : 'Orders over KRW 300,000 receive up to KRW 20,000 in delivery support.'}</strong></p><small>{ko ? '배송비 지원은 주문 금액 및 배송비 기준에 따라 적용되며, 지원 금액을 초과하는 배송비는 고객 부담입니다.' : 'Support is applied according to the order total and delivery fee. Any amount exceeding the supported fee is payable by the customer.'}</small></div></div>
}
...
function OrderInquiry({ ko }) {
  ...
  return <section className="order-inquiry">
    ...
    <details className="contact-delivery-fees"><summary>{ko ? '카카오 T 퀵 배송비 확인' : 'View Kakao T Quick Delivery Fees'} <b aria-hidden="true" /></summary><QuickDeliveryFeeGuide ko={ko} /></details>
