...
function Shop({ lang, detail }) {
  ...
  return <div className="page fade-in shop-page">
    ...
    <section className="shop-gallery-bridge container"><div className="shop-gallery-bridge-copy"><span className="eyebrow">Artificial Flower Gallery</span><h2>{ko ? <>시간이 지나도 아름다움이 오래도록 머무는<br />메이플레르의 프리미엄 조화 컬렉션을 만나보세요.</> : <>Discover Mayfleur’s premium artificial flower collection,<br />created to preserve its beauty through time.</>}</h2><p>{ko ? '다양한 플라워 스타일링과 디자인 작품을 갤러리에서 확인하실 수 있습니다.' : 'Explore a variety of floral styling and design works in the gallery.'}</p><a className="text-link" href="#gallery/artificial">{ko ? '조화 갤러리 보기' : 'View Artificial Flower Gallery'} →</a></div><div className="shop-gallery-bridge-images">{imagesOnly(galleryGroups.artificial).slice(0, 4).map((file, i) => <a href="#gallery/artificial" key={file}><Media file={file} alt={`${ko ? '조화 갤러리' : 'Artificial flower gallery'} ${i + 1}`} /></a>)}</div></section>
