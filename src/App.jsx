...
function FreshFlowerCollection({ lang }) {
  ...
  return <div className="page fade-in shop-page fresh-flower-page">
    ...
    <section className="fresh-flower-guide container">
      ...
      <FreshSection title="Delivery">{ko ? <><p>생화 상품은 <strong>카카오퀵 차량 배송으로 진행되며, 직접 픽업은 어렵습니다.</strong></p><p>배송비는 배송 거리에 따라 별도로 책정됩니다.</p><p><strong>원하시는 도착 시간을 기준으로 배송을 예약</strong>하며, 기사 배차 및 교통 상황에 따라 실제 도착 시간은 다소 앞당겨지거나 지연될 수 있습니다.</p></> : <><p>Fresh flower products are delivered <strong>by Kakao Quick vehicle service, and direct collection is unavailable.</strong></p><p>Delivery fees are calculated separately according to distance.</p><p>We <strong>schedule delivery based on your preferred arrival time</strong>, but the actual arrival may be slightly earlier or delayed depending on driver availability and traffic conditions.</p></>}</FreshSection>
